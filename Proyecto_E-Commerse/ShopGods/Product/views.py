...
def getProducts(amount_per_page: int = 25, actual_page: int = 1, search: str = '', username: str = None):
    try:
        ...
        page_amount = math.ceil(Product.objects.filter(
            condition).count() / amount_per_page)
        response = {
            'products': products,
    ...
    try:
        ...
        if type == 'key':
            items = Key.objects.filter(condition)[(
                amount_per_page*actual_page)-amount_per_page:amount_per_page*actual_page]
            page_amount = math.ceil(Key.objects.filter(
                condition).count() / amount_per_page)
        if type == 'mail':
            items = Mail.objects.filter(condition)[(
                amount_per_page*actual_page)-amount_per_page:amount_per_page*actual_page]
            page_amount = math.ceil(Mail.objects.filter(
                condition).count() / amount_per_page)
        if type == 'others':
            items = Others.objects.filter(condition)[(
                amount_per_page*actual_page)-amount_per_page:amount_per_page*actual_page]
            page_amount = math.ceil(Others.objects.filter(
                condition).count() / amount_per_page)
        response = {
            'items': items,
            'actual_page': actual_page,
...
def createProduct(product_param):
    try:
        ...
        product = Product(user=user, name=product_param["name"], price=product_param["price"], type=product_param["type"],
                          description=product_param["description"], photo=product_param["photo"] if product_param["photo"] is not None else 'static/product/default.jpg')
        product.save()
        return product
    except Exception as e:
        return False
...
def addItem(type, id_product, items):
    try:
        ...
        if type == 'mail':
            for mail in items:
                items_list.append(
                    Mail(mail=mail['mail'], password=mail['password'], product=product))
            Mail.objects.bulk_create(items_list)
        if type == 'others':
            for others in items:
                items_list.append(
                    Others(description=others['description'], product=product))
            Others.objects.bulk_create(items_list)
        if len(items_list) > 0:
            updateProductStock(product=product, add=True,
