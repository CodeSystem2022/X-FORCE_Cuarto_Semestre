...
def getShoppingCartByUsername(username: str):
    try:
        # Modificar por metodo creado en user
...
def getProductsOfShoppingCart(username: str):
    try:
        shoppingCartProducts = ShoppingCartProduct.objects.select_related(
            'shoppingCart__user').filter(shoppingCart__user__username=username)
...
def addProductToShoppingCart(username: str, product_param: any):
    try:
        ...
        condition = Q(product__id=product_param['id']) & Q(
            shoppingCart__user__username=username)
        ...
        if shoppingCartProduct:
            ...
            product_param['amount'] = product_param['amount'] + \
                shoppingCartProduct[0].amount
            thereIs = thereIsStock(
                product=product, amount=product_param['amount'])
        ...
        if thereIs['stock'] == True:
            ...
            shoppingCartProduct = ShoppingCartProduct(
                shoppingCart=shoppingCart, product=product, amount=product_param['amount'])
...
def deleteItemOfShoppingCart(id_shopping_cart_product: int):
    try:
        ShoppingCartProduct.objects.get(id=id_shopping_cart_product).delete()
        return True
    except Exception as e:
        return e
...
def deteleShoppingCart(username: str):
    try:
        response = ShoppingCart.objects.filter(
            user__username=username).delete()
        if response[0] > 0:
            return True
        if response[0] == 0:
            return False
